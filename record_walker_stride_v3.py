...
def record_episodes(
    ...
    if output_dir is None:
        output_dir = os.path.join(os.getcwd(), "videoswalkertrainedstridev3")
...
@hydra.main(
    ...
    config_name="dsrl_walker_stride_v3.yaml",  # <-- stride v3 config
    version_base=None,
)
def main(cfg: OmegaConf):
    """
    Example usage for Walker Stride v3:

        python record_walker_stride_v3.py \\
            model_path=/path/to/logs/gym-dsrl/gym_walker_dsrl_stride_v3_YYYY-MM-DD_HH-MM-SS_1/ \\
                     YYYY-MM-DD_HH-MM-SS_1/checkpoint/ft_policy_10000_steps.zip \\
            record_episodes=4

    This will save MP4s for episodes 0,1 and last two episodes into a `videoswalkertrainedstridev3/`
    folder inside the current directory (or whatever output_dir you set via video_dir).
    """
    ...
    output_dir = cfg.get("video_dir", os.path.join(os.getcwd(), "videoswalkertrainedstridev3"))
